      ...
      console.log(
       ...
       if(config.BACKUP.autobackup) {
        ...
        if(config.BACKUP.autosendwa) {
          try {
            await autoresbot.sendMessage(`${config.nomorsuperOwner}@s.whatsapp.net`, { document: fs.readFileSync('./backup.zip'), fileName: 'backup.zip', mimetype: 'application/zip' }).catch((err) => console.log('Failed Send Backup To Whatsapp'))
          } catch(error){
            console.log(error)
          }
        }
